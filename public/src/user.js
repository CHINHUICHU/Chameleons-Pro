...
$(document).ready(async () => {
  ...
  for (let i = 0; i < records.length; i += 1) {
    ...
    id="record-${i + 1}-similar-articles" disabled>
    ${records[i].similar_articles}</button></td>
    <td><button type="button" id="record-${i + 1}-highest-similarity" disabled>
    ${(records[i].highest_similarity * 100).toFixed(
      2
    )}%</button></td>`).appendTo(`#record-${i + 1}`);
